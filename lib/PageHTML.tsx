...
const SHARED_CHROME_CSS = [
  ...
  `.top .mark{flex:none;font-family:var(--display);letter-spacing:.02em;color:var(--ink);white-space:nowrap;font-size:22px;text-decoration:none}`,
  ...
  `.foot__soc{display:flex;flex-wrap:wrap;align-items:center;gap:10px;margin-inline-start:auto}`,
...
const REPORTS_CTA_CSS = [
  ...
  `@media (max-width: 480px){.rcta__btns{display:grid;grid-template-columns:1fr}}`,
].join("");
...
const TREATMENT_CONTENT_CSS = [
  ...
  `@media (max-width: 1100px){.keyfacts{grid-template-columns:repeat(2,1fr)}.proc__facts{grid-template-columns:1fr 1fr}.clauses{grid-template-columns:1fr}.clause{padding:22px}}`,
  `@media (max-width: 900px){.hero__grid{grid-template-columns:1fr;gap:40px}.rail{grid-template-columns:1fr;gap:18px}.rail__label{border-inline-start:0;border-block-start:2px solid var(--seal);padding:0 0 8px}.wgrid{grid-template-columns:1fr}.crit__row{gap:0 12px}}`,
  ...
  `@media (max-width: 520px){.keyfacts{grid-template-columns:1fr}.kf{padding:18px 16px}}`,
